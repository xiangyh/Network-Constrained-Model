##===============================================
##    Simulation for network-constrained model
##===============================================
...
column_size=0
beta_0p=c(0,rep(-1,3),rep(0,3),rep(0,3),rep(0,3),rep(-1,4),rep(0,4),rep(0,4),rep(0,4),rep(-1,5),rep(0,5),rep(0,5),rep(0,5))
...
for(iteration in 1:200){
  for(i in 1:10000){
    data=GenData(n,m,cor=0.6,inner_size,column_size,beta_0p)
    y=data[,dim(data)[2]]
    if(length(y[y==1])==n*0.1)
      break;
  }
  ...
  trainExtend=argumentMatrxIndependent(trainx[,-1],lambda2,sig=0);
  trainx.new=1/sqrt(1+lambda2) * rbind(scale(trainx[,-1]),trainExtend)
  trainy.new=c(trainy,rep(0,dim(trainExtend)[1]))
  train.new=data.frame(cbind(trainx.new,trainy.new)) # the new expanded data
  
  trainlasso1=glmnet(trainx.new,trainy.new,family="binomial")
  lambda=c(trainlasso1$lambda,0)
  trainlasso=glmnet(trainx.new,trainy.new,family="binomial",lambda=lambda)
  cvsim=cvlasso(trainx[,-1],trainy,lambda,blocks=5,lambda2,sig=0)   
  ...
  gmeancoef=1/sqrt(1+lambda2)*coef(trainlasso)[,gmean.lam]
  overallcoef=1/sqrt(1+lambda2)*coef(trainlasso)[,overall.lam]
  
  ## coefficients when cutoff=0.5
  gmeancoef2=1/sqrt(1+lambda2)*coef(trainlasso)[,gmean.lam2]
  overallcoef2=1/sqrt(1+lambda2)*coef(trainlasso)[,overall.lam2]
